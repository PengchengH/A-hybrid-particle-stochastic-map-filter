...
classdef TransportMapComponent
	...
	properties
        ...
        lambda    % L2 regularization parameter for opt (optimisation)
	...
	methods
        ...
        %TransportMapComponent: define TMc objective and initialise;
        %get_coeffs: check the map coffects including both diagnal and off-
        %ncoeff(TMc): check number of coeffects
        %set_Id_comp: set identity map component
        
        %evaluate(TMc, X): tramsform X through map
        %inverse(TMc, X, Z): tramsform X through inverse map
        
        %grad_x(TMc, X): gradient of map at point X
        %hess_x(TMc, X): Hessian of map at point X
        
        %optimize(TMc, X): get a set of map coffects by optimisation
           %setup_and_run_optim: % setup and run optimisation
           %objectiveOptim: % setup objective, gradient and hessian information for an
		...
		function TMc = TransportMapComponent(d,order,options,varargin)
            ...
            TMc.Diag.npoints_interp = options.npoints_interp; % number of points used for interpolation
            TMc.Diag.kappa          = options.kappa; % domain scaling for interpolating map
		...
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function coeffs = get_coeffs(TMc) % check coeffects
			coeffs = [TMc.OffD.coeffs; TMc.Diag.coeffs];
		end %endFunction
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function d = ncoeff(TMc)  % check number of coeffects
			d = TMc.OffD.ncoeff() + TMc.Diag.ncoeff();
		end %endFunction
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function TMc = set_Id_comp(TMc)   % set identity map component
            TMc.OffD = TMc.OffD.set_zero_function();
			TMc.Diag = TMc.Diag.set_Id_function();
		end %endFunction
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function Sx = evaluate(TMc, X)  % transport prior to standard normal
		...
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function Xk = inverse(TMc, X, Z)  % inverse map transform calculation
		...
		%------------------------------------------------------------------
		%------------------------------------------------------------------
        function dxS = grad_x(TMc, X)  %%gradient of map at point X
		...
		%------------------------------------------------------------------
		%------------------------------------------------------------------
        function d2xS = hess_x(TMc, X)  % Hessian of map at point X
        ...
        function TMc = optimize(TMc, X)
            ...
            Psi_mon  = TMc.Diag.basis_eval(X(:,end)); % Psi- a letter name
