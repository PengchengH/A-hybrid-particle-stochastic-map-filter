...
classdef MonotonePart
    ...
    %        order = 1 - linear
    %        order > 1 - (order+1) RBFs (radial basis function)
	...
	methods
        % MonotonePart: define class MonotonePart and give basic set
        % ncoeff: Calculate the number of coffects for monotone part(RBF+2erf)
        % set_Id_function: design identity map by linear map
        
        % evaluate: evaluate the value of RBF model based on the cached basis evaluations
        % grad_x: evaluate the gradient of monotone RBFs based on the cached basis
            % basis_eval:  % the value of basis (basic terms for monotone)on the 
                           % samples X, this is useful to calculate transport results
                           % and cost function.
            % basis_grad:  % the gradient of basis 
            % basis_hess:  % the Hessian of basis
                % eval_monotone_rbfs: evaluate the value of basis at samples X.
                % grad_x_monotone_rbfs: evaluate the gradient of basis at samples X.
                % hessian_x_monotone_rbfs: evaluate the Hessian of basis at samples X.
                
        % inverse: approximate inverse map of monotone part by interpolation
            % invert1D_map_interp: transform samples by inverse map
        % construct_basis: build centers and widths of basis functions
           % centers_and widths: detailed calculation for centers and width
        ...
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        function d = ncoeff(MP)  % number of ceffects for each term
        ...
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        function MP = set_Id_function(MP) % set identity linear map
        ...
        % evaluate the mon RBF model based on the cached basis evaluations
            ...
            cx = MP.basis_eval(X)*MP.coeffs; % Calculate the value of monotone part
        end %endFunction
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        ...
        % evaluate the grad of monotone RBFs based on the cached basis
            ...
            dcx = MP.basis_grad(X)*MP.coeffs; % Calculate the gradient of monotone part
        end %endFunction
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        function basisEval = basis_eval(MP, X)
            ...
            % evaluate monotone RBF terms for order > 1
                ...
                % the value of basis (basic terms for monotone)on the 
                % samples X, this is useful to calculate transport results
                % and cost function.
        ...
        function gradxEval = basis_grad(MP, X)
            ...
            % evaluate gradient of monotone RBF terms for order > 1
                ...
                % the gradient of basis (basic terms for monotone)on the 
                % samples X, this is useful to calculate transport results
                % and cost function.
        ...
        function hessxEval = basis_hess(MP, X)
            ...
            % evaluate hessian of monotone RBF terms for order > 1
                ...
                % the Hessian of basis (basic terms for monotone)on the 
                % samples X, this is useful to calculate transport results
                % and cost function.
        ...
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        function Xk = inverse(MP, Z)  % approximate inverse map by interpolation
        ...
        function MP = construct_basis(MP, X)
            %build centers and widths of basis functions
        ...
        function f = eval_monotone_rbfs(MP, X, centers, widths)
            % evaluate the value of basis at samples X.
        ...
        function df = grad_x_monotone_rbfs(MP, X, centers, widths)
            % evaluate the gradient of basis at samples X.
        ...
        function d2f = hess_x_monotone_rbfs(MP, X, centers, widths)
            % evaluate the hessian of basis at samples X.
